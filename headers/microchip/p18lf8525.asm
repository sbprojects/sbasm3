...
;  MPASM PIC18LF8525 processor include
;
...
;  bits of information for the PIC18LF8525 microcontroller.  These names
...
;
;   ECCP Mux bit:
;     ECCPMX = PORTH       ECCP1 (P1B/P1C) and ECCP3 (P3B/P3C) PWM outputs are multiplexed with RH7 through RH4
;     ECCPMX = PORTE       ECCP1 (P1B/P1C) and ECCP3 (P3B/P3C) PWM outputs are multiplexed with RE6 through RE3
;
;   MCLR Enable bit:
...
_OSC_RC_1H           .EQ  $F3  RC oscillator with OSC2 configured as divide by 4 clock output
_OSC_EC_1H           .EQ  $F4  EC oscillator with OSC2 configured as divide by 4 clock output
_OSC_ECIO_1H         .EQ  $F5  EC oscillator with OSC2 configured as RA6
_OSC_HSPLL_1H        .EQ  $F6  HS oscillator with HW enabled 4x PLL
_OSC_RCIO_1H         .EQ  $F7  RC oscillator with OSC2 configured as RA6
_OSC_ECIOPLL_1H      .EQ  $FC  EC oscillator with OSC2 configured as RA6 and HW enabled 4x PLL
_OSC_ECIOSWPLL_1H    .EQ  $FD  EC oscillator with OSC2 configured as RA6 and SW enabled 4x PLL
_OSC_HSSWPLL_1H      .EQ  $FE  HS oscillator with SW enabled 4x PLL
...
_WAIT_ON_3L          .EQ  $7F  Wait selections for table reads and table writes are determined by WAIT1:WAIT0 bits (MEMCOM<5:4>)
_WAIT_OFF_3L         .EQ  $FF  Wait selections unavailable for table reads and table writes

;----- CONFIG3H Options --------------------------------------------------
_CCP2MX_PORTBE_3H    .EQ  $FE  Multiplexed with RB3 or RE7
_CCP2MX_PORTC_3H     .EQ  $FF  Multiplexed with RC1

_ECCPMX_PORTH_3H     .EQ  $FD  ECCP1 (P1B/P1C) and ECCP3 (P3B/P3C) PWM outputs are multiplexed with RH7 through RH4
_ECCPMX_PORTE_3H     .EQ  $FF  ECCP1 (P1B/P1C) and ECCP3 (P3B/P3C) PWM outputs are multiplexed with RE6 through RE3

_MCLRE_OFF_3H        .EQ  $7F  RG5 input enabled, MCLR disabled
